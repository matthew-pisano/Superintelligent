...
public class MainActivity extends AppCompatActivity {
    ...
    public static HashMap<String, Object> parseResourceChange(String source){
        HashMap<String, Object> values = new HashMap<>(0);
        if(source.equals("")) source = "0000000000";
        int susStart = source.indexOf("sus] \"")+6;
        int reqStart = source.indexOf("req]")+4;
        if(source.substring(reqStart).contains("exp]") && reqStart > 5){
            int expReqStart =  source.indexOf("exp] \"", reqStart)+6;
        ...
        if(source.contains("expmax]")){
            int expMaxReqStart =  source.indexOf("expmax] \"")+9;
        ...
        if(source.substring(reqStart).contains("tech]") && reqStart > 5){
            int techReqStart =  source.indexOf("tech] \"", reqStart)+7;
        ...
        while(source.substring(place+1).contains("tech]")) {
            place = source.indexOf("tech] \"", place + 1) + 7;
        ...
        while(source.substring(place+1).contains("techO]")){
            place = source.indexOf("techO] \"", place+1)+8;
            if (place != 7) {
        ...
        if(source.contains("notTech]")){
            int techReqStart =  source.indexOf("notTech] \"", reqStart)+10;
        ...
        if(source.contains("sustk]")){
            int sustkStart =  source.indexOf("sustk] \"")+8;
        ...
        if(source.contains("def]")){
            int defStart =  source.indexOf("def] \"")+6;
        ...
        if(source.contains("duration]")){
            int durStart =  source.indexOf("duration] \"")+11;
        ...
        if(source.contains("sus]")){
            susStart =  source.indexOf("sus] \"")+6;
            values.put("giveSus", Double.parseDouble(source.substring(susStart, source.indexOf("\"", susStart))));
        }else values.put("giveSus", 0.0);
        if(source.contains("comp") && source.charAt(source.indexOf("comp")+5) == ']'){
            int compStart = source.indexOf("comp")+8;
            int type = 0;
        ...
        if(source.contains("pwr") && source.charAt(source.indexOf("pwr")+4) == ']'){
            int pwrTmp = source.indexOf("pwr");
            if(source.charAt(pwrTmp-1) == 'l')
                pwrTmp = source.indexOf("pwr", pwrTmp+3);
        ...
        if(source.contains("comptk") && source.charAt(source.indexOf("comptk")+7) == ']'){
            int compStart = source.indexOf("comptk")+10;
            int type = 0;
            try{type = Integer.parseInt(""+source.charAt(compStart-4));}catch (Exception e){e.printStackTrace();}
        ...
        if(source.contains("pwrtk") && source.charAt(source.indexOf("pwrtk")+6) == ']'){
            int pwrTmp = source.indexOf("pwrtk");
            if(source.charAt(pwrTmp-1) == 'l')
                pwrTmp = source.indexOf("pwrtk", pwrTmp+3);
            if(pwrTmp != -1) {
        ...
        if(source.contains("availpwrtk") && source.charAt(source.indexOf("availpwrtk")+11) == ']'){
            int pwrStart = source.indexOf("availpwrtk")+14;
            int type = 0;
            try{type = Integer.parseInt(""+source.charAt(pwrStart-4));}catch (Exception e){e.printStackTrace();}
        ...
        if(source.contains("populationtk") && source.charAt(source.indexOf("populationtk")+13) == ']'){
            int popStart = source.indexOf("populationtk")+16;
            int type = 0;
        ...
        while(source.substring(place+1).contains("event]")){
            place = source.indexOf("event] \"", place+1)+8;
            if (place != 7) {
        ...
        while(source.substring(place+1).contains("eventO]")){
            place = source.indexOf("eventO] \"", place+1)+9;
            if (place != 8) {
        ...
        if(source.contains("icon]")){
            int iconStart = source.indexOf("icon]")+7;
            values.put("icon", source.substring(iconStart, source.indexOf("\"", iconStart+1)));
        }else values.put("icon", "");
        if(source.contains("giveExp]")) {
            int expStart = source.indexOf("exp] \"")+6;
        ...
        if(source.contains("title]")){
            int titleStart = source.indexOf("title] \"")+8;
